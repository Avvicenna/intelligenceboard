...
async function auditRoute(page, route) {
  ...
  // Collect all elements and check computed font-family (literal RegExp in page context — avoids non-literal RegExp Semgrep finding)
  const violations = await page.evaluate(() => {
    const re = /monospace|courier|menlo|monaco|consolas|sfmono|fira.?code|ibm.?plex.?mono|geist.?mono/i
    const results = []
